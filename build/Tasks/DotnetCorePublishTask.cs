...
public sealed class DotnetCorePublishTask : FrostingTask<BuildContext>
{
    public override void Run(BuildContext context)
    {
        ...
        {
            if (!projectSetting.PublishProject) continue;
            var outputDirectory = Path.Combine(context.Config.StandardFolders.PublishDir, projectSetting.ProjectName);
            ...
            {
                context.Information(
                    $"dotnet publish - framework set as {context.Config.DotnetSettings.DotnetFramework}.");
            ...
            context.DotNetPublish(context.GetProjectFilePathUsingBuiltinArguments(projectSetting.ProjectName),
                settings);
        }
