...
public sealed class DockerEcrPushTask : FrostingTask<BuildContext>
{
    public override void Run(BuildContext context)
    {
        ...
        {
            if (projectSetting.DockerImageSettings == null || !projectSetting.DockerImageSettings.Any()) continue;
            foreach (var dockerSetting in projectSetting.DockerImageSettings)
                if (dockerSetting.RunDockerPush)
                {
                    var awsApp = context.Config.AwsApplication;
                    context.DockerEcrLogin(dockerSetting.AwsAccountNumber, dockerSetting.AwsRegion);
                    var buildTagName =
                        context.Config.AwsApplication.GetResourceName(dockerSetting.DockerRepoNameSuffix);
                    var ecrTagNameBase =
                        $"{dockerSetting.AwsAccountNumber}.dkr.ecr.{dockerSetting.AwsRegion}.amazonaws.com/{buildTagName}";
