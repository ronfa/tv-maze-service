...
{
    public static string BuildDefaultAppResourceCondition(this AWSAppProject awsApplication, Stack stack,
        string serviceName, string policyPrefix)
    {
    ...
    public static string? GetAppResourceCondition(this AWSAppProject awsApplication, Stack stack,
        AwsResourceType resourceType)
    {
        switch (resourceType)
        {
            ...
            {
                return awsApplication.BuildDefaultAppResourceCondition(stack, "sqs",
                    awsApplication.IAMRolePolicyPrefix());
            }

            case AwsResourceType.SNS:
            {
                return awsApplication.BuildDefaultAppResourceCondition(stack, "sns",
                    awsApplication.IAMRolePolicyPrefix());
            }
            ...
            {
                return
                    $"arn:aws:ssm:{stack.Region}:{stack.Account}:parameter/{awsApplication.IAMRolePolicyPrefix("/")}*";
            }
            ...
            {
                return
                    $"arn:aws:logs:{stack.Region}:{stack.Account}:log-group:/aws/lambda/{awsApplication.IAMRolePolicyPrefix()}*";
            }
            case AwsResourceType.CloudWatchLogGroup:
            {
                return
                    $"arn:aws:logs:{stack.Region}:{stack.Account}:log-group:/*{awsApplication.IAMRolePolicyPrefix()}*";
            }
            case AwsResourceType.CodeBuildReportGroup:
            {
                return
                    $"arn:aws:codebuild:{stack.Region}:{stack.Account}:report-group:/{awsApplication.IAMRolePolicyPrefix()}*";
            }
        }
    ...
    private static string[] GetProcessedResource(this Role role, AWSAppProject awsApplication,
        AwsResourceType resourceType, string[]? conditions = null)
    {
        var resources = new List<string>
        {
            awsApplication.GetAppResourceCondition(role.Stack, resourceType)!
        };
        if (conditions?.Any() == true) resources.AddRange(conditions);
    ...
    public static Role AddSsmPolicy(this Role role, AWSAppProject awsApplication, string[]? conditions = null)
    {
        var processedResources =
            role.GetProcessedResource(awsApplication, AwsResourceType.SSMParameterStore, conditions);
        role.AddToPolicy(new PolicyStatement(new PolicyStatementProps
        {
            Actions = new[]
            {
    ...
    public static Role AddSqsPolicy(this Role role, AWSAppProject awsApplication, string[]? conditions = null)
    {
        ...
        role.AddToPolicy(new PolicyStatement(new PolicyStatementProps
        {
            Actions = new[]
            {
                "sqs:ChangeMessageVisibility", "sqs:DeleteMessage", "sqs:GetQueue*", "sqs:PurgeQueue",
    ...
    public static Role AddCloudWatchLogsPolicy(this Role role, AWSAppProject awsApplication,
        string[]? conditions = null)
    {
        var processedResources =
            role.GetProcessedResource(awsApplication, AwsResourceType.CloudWatchLambdaLogs, conditions);
        role.AddToPolicy(new PolicyStatement(new PolicyStatementProps
        {
            Actions = new[]
            {
    ...
    public static Role AddCloudWatchLogGroupPolicy(this Role role, AWSAppProject awsApplication,
        string[]? conditions = null)
    {
        var processedResources =
            role.GetProcessedResource(awsApplication, AwsResourceType.CloudWatchLogGroup, conditions);
        role.AddToPolicy(new PolicyStatement(new PolicyStatementProps
        {
            Actions = new[]
            {
    ...
    public static Role AddCodeBuildReportGroupPolicy(this Role role, AWSAppProject awsApplication,
        string[]? conditions = null)
    {
        var processedResources =
            role.GetProcessedResource(awsApplication, AwsResourceType.CodeBuildReportGroup, conditions);
        role.AddToPolicy(new PolicyStatement(new PolicyStatementProps
        {
            Actions = new[]
            {
