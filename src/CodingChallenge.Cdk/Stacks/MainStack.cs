...
{
    public MainStack(Construct parent, string id, IStackProps props, AWSAppProject awsApplication) : base(parent, id,
        props)
    {
        if (string.IsNullOrWhiteSpace(awsApplication.AwsRegion)) awsApplication.AwsRegion = Region;

        var eventTopic = GetTopicReference(awsApplication);
    ...
    private ITopic GetTopicReference(AWSAppProject awsApplication)
    {
        ...
        var eventTopic = Topic.FromTopicArn(this, awsApplication.GetResourceName("blockChainEventTopic"),
            awsApplication.GetCfOutput($"{InfraStack.eventTopicSuffix}-{InfraStack.arnSuffixValue}"));
        return eventTopic;
    }
    ...
    private void SetupEventProcessor(AWSAppProject awsApplication, ITopic eventTopic)
    {
        ...
        new EventProcessorNestedStack(this, "eventprocessor", new NestedStackProps(), awsApplication,
            eventProcessorLambdaRole.RoleArn, eventTopic.TopicArn);
    }
    ...
    private void SetupApiGateway(AWSAppProject awsApplication, ITopic eventTopic)
    {
        ...
        new ApiGatewayNestedStack(this, "ApiGateway", new NestedStackProps(), awsApplication, eventTopic.TopicArn,
            apiGw.RestApiId, apiGw.Root.ResourceId, apiGwRole.RoleArn);
    }

    private void SetCustomDomainForApiGateway(AWSAppProject awsApplication, RestApi restapi)
    {
        var domainNameObj = new CfnDomainName(this, awsApplication.GetResourceName($"{restapi.RestApiName}-domainname"),
        ...
        var basePathMapping = new CfnBasePathMapping(this,
            awsApplication.GetResourceName($"{restapi.RestApiName}-basemappings"), new CfnBasePathMappingProps()
            {
    ...
    {
        return awsApplication.GetAPIGatewayRole(this, namesuffix)
